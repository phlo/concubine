...
\newcommand{\ccASM}[1]{\texttt{\color{gray}#1}}
...
std::map<State, std::vector<std::vector<uint>>> updates = {
  {State::accu, {{0, 2},    // `\color{gray}$\ACCU_0 \leftarrow$ ADDI 1, LOAD 0`
                 {0, 2},    // `\color{gray}$\ACCU_1 \leftarrow$ ADDI 1, LOAD 1`
                 {1, 2}}},  // `\color{gray}$\ACCU_2 \leftarrow$ ADD 0, ADD 1`
  {State::adr,  {{1},       // `\color{gray}$\SBADR_0 \leftarrow$ STORE 0`
                 {1},       // `\color{gray}$\SBADR_1 \leftarrow$ STORE 1`
                 {}}},      // `\color{gray}$\SBADR_2 \leftarrow$ $\varnothing$`
  {State::val,  {{1},       // `\color{gray}$\SBVAL_0 \leftarrow$ STORE 0`
                 {1},       // `\color{gray}$\SBVAL_1 \leftarrow$ STORE 1`
                 {}}},      // `\color{gray}$\SBVAL_2 \leftarrow$ $\varnothing$`
  {State::full, {{1},       // `\color{gray}$\SBFULL_0 \leftarrow$ STORE 0`
                 {1},       // `\color{gray}$\SBFULL_1 \leftarrow$ STORE 1`
                 {}}},      // `\color{gray}$\SBFULL_2 \leftarrow$ $\varnothing$`
  {State::halt, {{4},       // `\color{gray}$\HALT_0 \leftarrow$ HALT`
                 {4},       // `\color{gray}$\HALT_1 \leftarrow$ HALT`
                 {}}},      // `\color{gray}$\HALT_2 \leftarrow$ $\varnothing$`
  {State::exit, {{},        // `\color{gray}$\EXIT \leftarrow$ $\varnothing$`
                 {},        // `\color{gray}$\EXIT \leftarrow$ $\varnothing$`
                 {4, 5}}}}; // `\color{gray}$\EXIT \leftarrow$ EXIT 0, EXIT 1`
\end{lstlisting}
% \vspace{-\baselineskip}
...
std::vector<std::vector<std::vector<uint>>> predecessors = {
  // thread 0
  {{},    // `\color{gray}ADDI 1`    `\color{gray}$\leftarrow$ $\varnothing$`
   {0},   // `\color{gray}STORE 0`   `\color{gray}$\leftarrow$ ADDI 1`
   {1},   // `\color{gray}LOAD 1`    `\color{gray}$\leftarrow$ STORE 0`
   {2},   // `\color{gray}CHECK 0`   `\color{gray}$\leftarrow$ LOAD 1`
   {3}},  // `\color{gray}HALT`      `\color{gray}$\leftarrow$ CHECK 0`
  // thread 1
  {{},    // `\color{gray}ADDI 1`    `\color{gray}$\leftarrow$ $\varnothing$`
   {0},   // `\color{gray}STORE 1`   `\color{gray}$\leftarrow$ ADDI 1`
   {1},   // `\color{gray}LOAD 0`    `\color{gray}$\leftarrow$ STORE 1`
   {2},   // `\color{gray}CHECK 0`   `\color{gray}$\leftarrow$ LOAD 0`
   {3}},  // `\color{gray}HALT`      `\color{gray}$\leftarrow$ CHECK 0`
  // thread 2
  {{},    // `\color{gray}CHECK 0`   `\color{gray}$\leftarrow$ $\varnothing$`
   {0},   // `\color{gray}ADD 0`     `\color{gray}$\leftarrow$ CHECK 0`
   {1},   // `\color{gray}ADD 1`     `\color{gray}$\leftarrow$ ADD 0`
   {2},   // `\color{gray}JZ error`  `\color{gray}$\leftarrow$ ADD 1`
   {3},   // `\color{gray}EXIT 0`    `\color{gray}$\leftarrow$ JZ error`
   {3}}}; // `\color{gray}error:` `\color{gray}EXIT 1` `\color{gray}$\leftarrow$ JZ error`
\end{lstlisting}
...
std::vector<std::vector<uint>> barriers = {
  {1, 4}, // thread 0: `\color{gray}STORE 0, HALT`
  {1, 4}, // thread 1: `\color{gray}STORE 1, HALT`
  {}};    // thread 2: `\color{gray}$\varnothing$`
\end{lstlisting}
...
std::map<uint, std::map<uint, std::vector<uint>>> checkpoints = {
  // checkpoint 0
  {0, {{0, {3}},    // thread 0: `\color{gray}CHECK 0`
       {1, {3}},    // thread 1: `\color{gray}CHECK 0`
       {2, {0}}}}}; // thread 2: `\color{gray}CHECK 0`
\end{lstlisting}
\end{itemize}
