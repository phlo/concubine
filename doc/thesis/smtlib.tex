...
Each available instruction is represented by its own type, derived from an abstract class \lstCPP{Instruction}, capturing arguments and abolishing the need for lengthy case splits by utilizing dynamic dispatch of virtual member functions for taking the appropriate action at runtime.%determine the type of instruction at runtime.
...
\begin{tabu}{|c|c|c|}
  ...
  \lasthline
\end{tabu}
\caption{Store Buffer Litmus Test Programs and Activation Variables}
...
\begin{tabu} to \textwidth {|X|l|X|}
  ...
  \lasthline
\end{tabu}
\caption{\SMTLIB{} Expression Generator Functions}
