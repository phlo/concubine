...
Access to uninitialized memory cells is captured in so called memory maps\footnote{\CHANGE{naming inspired by, but not to be confused with memory mapped I/O}} again a simple text file format, containing one address value pair per line, delimited by a whitespace.
...
After a couple of simulations, a trace like the one shown in Listing \ref{lst:trace:error}, where none of the store buffers has been flushed prior to the \emph{checker threads} \lstASM{ADD} instructions, will be found.
