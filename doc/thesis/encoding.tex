...
relying on \texttt{ite} cascades to perform \emph{static single assignments} for determining the successor of each state in a functional manner.% and aids the solver by simplifying the deduction process.
...
% in order to show the differences encoding in terms of the
...
~\\
\noindent
\lstinline[style=c++]{std::string Instruction::encode (uint t, uint k, uint pc)}
~\\
