...
\todo[inline]{Successor State Axioms?}\noindent
...
To simplify the definition of axioms two  $\EFFECTIVE^k: \BVSORT \to \BVSORT$
\todo[inline]{
\[
  \EFFECTIVE^k(adr) =
  \begin{cases}
...
\begin{longtabu}{llX[m]}
  ...
  \multirow{2}{*}{\texttt{FENCE}}
    ...
    & $\STMT^k_{t, pc + 1}$ & $= true$ \\
  \hline
  \multirow{3}{*}{\texttt{ADD adr}}
    & $\ACCU^{k + 1}_t$     & $= \ACCU^k_t + \LOAD^k_t(\texttt{adr})$ \\
    & $\STMT^k_{t, pc}$     & $= false$ \\
    & $\STMT^k_{t, pc + 1}$ & $= true$ \\
  \hline
  \multirow{3}{*}{\texttt{ADDI val}}
    & $\ACCU^{k + 1}_t$     & $= \ACCU^k_t + \texttt{val}$ \\
    & $\STMT^k_{t, pc}$     & $= false$ \\
    & $\STMT^k_{t, pc + 1}$ & $= true$ \\
  \hline
  \multirow{3}{*}{\texttt{SUB adr}}
    & $\ACCU^{k + 1}_t$     & $= \ACCU^k_t - \LOAD^k_t(\texttt{adr})$ \\
    & $\STMT^k_{t, pc}$     & $= false$ \\
    & $\STMT^k_{t, pc + 1}$ & $= true$ \\
  \hline
  \multirow{3}{*}{\texttt{SUBI val}}
    & $\ACCU^{k + 1}_t$     & $= \ACCU^k_t - \texttt{val}$ \\
    & $\STMT^k_{t, pc}$     & $= false$ \\
    & $\STMT^k_{t, pc + 1}$ & $= true$ \\
  \hline
  \multirow{3}{*}{\texttt{MUL adr}}
    & $\ACCU^{k + 1}_t$     & $= \ACCU^k_t * \LOAD^k_t(\texttt{adr})$ \\
    & $\STMT^k_{t, pc}$     & $= false$ \\
    & $\STMT^k_{t, pc + 1}$ & $= true$ \\
  \hline
  \multirow{3}{*}{\texttt{MULI val}}
    & $\ACCU^{k + 1}_t$     & $= \ACCU^k_t * \texttt{val}$ \\
    & $\STMT^k_{t, pc}$     & $= false$ \\
    & $\STMT^k_{t, pc + 1}$ & $= true$ \\
  \hline
  \multirow{3}{*}{\texttt{CMP adr}}
    & $\ACCU^{k + 1}_t$     & $= \ACCU^k_t - \LOAD^k_t(\texttt{adr})$ \\
    & $\STMT^k_{t, pc}$     & $= false$ \\
    & $\STMT^k_{t, pc + 1}$ & $= true$ \\
  \hline
  ...
  \multirow{3}{*}{\texttt{JS arg}}
    & $\STMT^k_{t, pc}$     & $= false$ \\
    % & $\STMT^k_{t, pc + 1}$ & $= \lnot (\ACCU^k_t \mathbin{\&} (1 \ll 15))$ \todo[noline]{shift?} \\
    & $\STMT^k_{t, pc + 1}$ & $= \lnot \MSB(\ACCU^k_t)$ \\
    % & $\STMT^k_{t,\texttt{arg}}$ & $= \ACCU^k_t \mathbin{\&} (1 \ll 15)$ \\
    & $\STMT^k_{t,\texttt{arg}}$ & $= \MSB(\ACCU^k_t)$ \\
  \hline
  \multirow{3}{*}{\texttt{JNS arg}}
    & $\STMT^k_{t, pc}$     & $= false$ \\
    % & $\STMT^k_{t, pc + 1}$ & $= \ACCU^k_t \mathbin{\&} (1 \ll 15)$ \\
    & $\STMT^k_{t, pc + 1}$ & $= \MSB(\ACCU^k_t)$ \\
    % & $\STMT^k_{t,\texttt{arg}}$ & $= \lnot (\ACCU^k_t \mathbin{\&} (1 \ll 15))$ \\
    & $\STMT^k_{t,\texttt{arg}}$ & $= \lnot \MSB(\ACCU^k_t)$ \\
  \hline
  \multirow{3}{*}{\texttt{JNZNS arg}}
    & $\STMT^{k + 1}_{t, pc}$     & $= false$ \\
    % & $\STMT^k_{t, pc + 1}$ & $= \lnot \ACCU^k_t \lor (\ACCU^k_t \mathbin{\&} (1 \ll 15))$ \\
    & $\STMT^{k + 1}_{t, pc + 1}$ & $= \lnot \ACCU^k_t \lor \MSB(\ACCU^k_t)$ \\
    % & $\STMT^k_{t,\texttt{arg}}$ & $= \ACCU^k_t \land \lnot (\ACCU^k_t \mathbin{\&} (1 \ll 15))$ \\
...
% \vspace{0.1cm}
% \noindent
% \begin{tabu} to \textwidth {lX[m]}
  % $\HEAP^{k + 1}$       & $= \WRITE^k(\SBADR^k_t, \SBVAL^k_t)$ \\
  % $\SBFULL^{k + 1}$     & $= false$ \\
% \end{tabu}

% \paragraph{\texttt{FLUSH}}
