...
void define_sb_full (uint k)
{
  for (const auto & [t, stmts] : updates[State::sb_full])
    {
      std::vector<std::string> args {sb_full_var(k - 1, t)};
      for (uint pc : stmts)
    ...
    for (uint pc = 0; pc < programs[t].size(); pc++)
      {
        ...
        for (auto pre = stmts.rbegin(); pre != stmts.rend(); ++pre)
          {
            std::string val = exec_var(k - 1, t, *pre);
            Instruction * op = programs[t][*pre];
            // add condition if predecessor is a jump
            if (is_jump(op))
              {
            ...
            expr = ite(stmt_var(k - 1, t, *pre), val, expr);
          }
        formula << assign(stmt_var(k, t, pc), expr) << '\n';
      }
}
...
void define_block (uint k)
{
  for (const auto & [id, threads] : checkpoints)
    for (const auto & [t, stmts] : threads)
      {
        std::vector<std::string> args {block_var(k - 1, id, t)};
        for (uint pc : stmts)
          args.push_back(exec_var(k - 1, t, pc));
...
void define_halt (uint k)
{
  for (const auto & [t, stmts] : updates[State::halt])
    {
      std::vector<std::string> args {halt_var(k - 1, t)};
      for (uint pc : stmts)
        args.push_back(exec_var(k - 1, t, pc));
      formula << assign(halt_var(k, t), lor(args)) << '\n';
    }
}
\end{lstlisting}
  ...
  for (int t = programs.size() - 1; t >= 0; t--)
    {
      ...
      for (auto pc = stmts.rbegin(); pc != stmts.rend(); ++pc)
        expr = ite(exec_var(k - 1, t, *pc),
                   ...
                   expr);
      expr = ite(flush_var(k - 1, t),
                 store(heap_var(k - 1),
                       sb_adr_var(k - 1, t),
                       sb_expr_var(k - 1, t)),
                 expr);
    }
  formula << assign(heap_var(k, t), expr) << '\n';
}
\end{lstlisting}
...
void define_exit_flag (uint k)
{
  ...
  for (const auto & [t, _] : updates[State::halt])
    halts.push_back(halt_var(k, t));
  std::vector<std::string> args {exit_flag_var(k - 1), land(halts)};
  for (const auto & [t, stmts] : updates[State::exit])
    for (uint pc : stmts)
      args.push_back(exec_var(k - 1, t, pc));
  formula << assign(exit_flag_var(k), lor(args)) << '\n';
}
\end{lstlisting}

\noindent
\emph{Example:} \lstinline[style=c++]{define_exit_flag(1)}
...
% again relies on
is again based on
dynamically dispatched member functions of concrete \texttt{Instruction} class instantiations.
...
{
  Next next(k, t);
  next[State::accu] = equality(accu(k, t),
...
\noindent
Enable the corresponding blocking flag $\BLOCK^k_{id, t}$, while preserving or resetting any other.
...
{
  for (uint pc = 0; pc < programs[t].size(); pc++)
    formula << imply_next(exec_var(k - 1, t, pc),
...
void imply_thread_not_executed (uint k, uint t)
{
  ...
  formula << imply_next(lnot(thread_var(k - 1, t)), next) << '\n';
}
\end{lstlisting}
...
{
  formula << imply_next(flush_var(k - 1, t),
...
{
  formula << imply_next(exit_flag_var(k - 1),
                        land(restore(&heap_var, k),
                             exit_flag_var(k))) << '\n';
  // set exit if machine didn't exit within bound
  if (k == bound)
    formula << imply_next(lnot(exit_flag_var(k)),
                          equality(exit_code_var, consth(0))) << '\n';
}
\end{lstlisting}
